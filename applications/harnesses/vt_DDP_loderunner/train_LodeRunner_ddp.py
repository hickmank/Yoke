"""DDP training harness for LodeRunner on lsc240420 (vt variant).

Thin wrapper around :class:`yoke.harnesses.trainer.HarnessTrainer`. Identical in
structure to the ``se_DDP_loderunner`` harness but uses ``anchor_lr`` as both the
optimizer learning rate and the constant-with-warmup scheduler level.
"""
...
def build_scheduler(
    ...
    """Build the constant-with-warmup LR scheduler at ``anchor_lr``.
