...
NOTE: This harness previously contained a latent bug where the checkpoint was
saved with ``model_class=LodeRunner`` despite constructing a ``LodeRunnerViT``.
Because ``HarnessTrainer`` derives ``model_class`` from the model actually
built, the checkpoint now correctly records ``LodeRunnerViT``.
"""
